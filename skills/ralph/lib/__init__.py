...
The subsequent `refactor/ralph-babysit-collapse` collapse folded both
promoted modules into `lib/ralph_chain.py` (lib.ralph_state.py was
deleted). lib/ is the canonical SSOT surface for shared Python helpers;
the skills/ namespace was a parallel hierarchy that forced every
...
This shim re-exports the symbol surface of the merged
`lib.ralph_chain` module so legacy call sites of the form
...
runtime. New code MUST use the top-level path:
    ...
    from lib.ralph_chain import RalphState   # or `python3 -m lib.ralph_chain`

Note: legacy code that did `from skills.ralph.lib import ralph_chain`
(the module form, not the symbol form) was updated as part of the
collapse — see the audit commit. The module-attribute shim is no
longer needed because the form `from skills.ralph.lib import
ralph_state` was only used by `tests/test_ralph_controller.py`,
which now reads `from lib import ralph_chain as ralph_state`. No
...
`skills/ralph/lib/__init__.py`, so a bare `from lib.ralph_chain`
...
_warnings.warn(
    "skills.ralph.lib is deprecated; import from lib.ralph_chain instead.",
    DeprecationWarning,
    stacklevel=2,
)

# _Prepend the project-root-anchored lib/ to sys.path so the
# `from lib.ralph_chain` imports below resolve to the canonical
