#!/usr/bin/env python3
"""role_config.py — operator-declared `roles` block for `DEV_KIT_MODE=mod`.

`mod` is the fourth mode added in this PR. It activates two
...
role names + skill subsets at `/dev-kit:mode mod` time. This module is
...
- `RoleConfigError` — raised when a `roles` block exists but
  `DEV_KIT_MODE != mod` (the gate is fail-closed).
...
operator picked `--mode mod`. If a downstream consumer (a hook, a
...
`/dev-kit:mode mod` picker loop (see `skills/mode/SKILL.md`).
"""
...
class RoleConfigError(Exception):
    ...
    `.claude/settings.json` but `DEV_KIT_MODE` is not `mod`. Either the
    mode was switched away from `mod` (and the roles block should be
    removed) or something wrote `roles` outside the `/dev-kit:mode mod`
...
# it is the set the `mod` role-wildcard expands to. Operators wanting
...
_VALID_MODES = frozenset({"full", "lite", "undev", "mod"})
...
def _active_mode(root: Path) -> Optional[str]:
    ...
    if we read `mod` here but the CLI resolves `undev` because of a
...
def _enforce_mode_gate(root: Path) -> None:
    """Raise `RoleConfigError` if a `roles` block exists but mode != mod.

    The role block is honored only in `mod` mode. Reading it from any
    ...
    raise RoleConfigError(
        ...
        f"DEV_KIT_MODE={mode!r}. Switch to DEV_KIT_MODE=mod (via "
        f"`/dev-kit:mode mod`) or remove the `roles` block."
    )
