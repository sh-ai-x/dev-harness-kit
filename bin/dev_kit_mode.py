...
def _resolve_mode(cwd: Path) -> tuple[str, str]:
    ...
    try:
        ...
        if shell_mode in {"full", "lite", "undev", "mod"}:
...
def cmd_write(args) -> int:
    if not args.mode:
        ...
        return 2
    if args.mode not in {"full", "lite", "undev", "mod"}:
        print(f"error: invalid mode {args.mode!r}; must be full|lite|undev|mod",
              file=sys.stderr)
        return 2
...
def main(argv: list[str] | None = None) -> int:
    ...
    p_write.add_argument("--mode", choices=["full", "lite", "undev", "mod"])
