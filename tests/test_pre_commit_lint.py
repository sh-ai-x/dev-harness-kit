...
class TestPreCommitLint(unittest.TestCase):
    ...
    def test_ignores_unstaged_tracked_python_when_text_file_is_staged(self):
        if shutil.which("ruff") is None:
            self.skipTest("ruff is not installed")
        with _init_tmp_git_repo() as directory:
            root = Path(directory)
            (root / "old.py").write_text("import os\n")
            subprocess.run(["git", "-C", str(root), "add", "old.py"], check=True)
            subprocess.run(["git", "-C", str(root), "commit", "-q", "-m", "old"], check=True)
            (root / "notes.txt").write_text("only this file is staged\n")
    ...
    def test_allows_markdown_setext_heading(self):
        """A long Markdown underline is not a Git conflict marker."""
        with _init_tmp_git_repo() as directory:
            root = Path(directory)
    ...
    def test_lints_staged_file_with_special_chars_in_name(self):
        ...
        with _init_tmp_git_repo() as directory:
            ...
            for special_name in ('quote"file.py', "with space.py"):
                ...
                result = _run_hook(root)
                self.assertNotIn(
                    "unable to read staged Python blobs",
                    result.stderr,
                    f"special filename broke checkout-index: {result.stderr!r}",
                )
