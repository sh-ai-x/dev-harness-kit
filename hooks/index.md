...
> The regen tool is run on every `SessionStart` by the
> `hooks/session-start.sh` dispatcher, which invokes
> `hooks/session-start-check.sh` as one of its child modules. It preserves any pre-existing
...
The runtime manifests register 27 command entries each. Before the thin
registration pass there were 36: eight SessionStart entries were reduced to
one dispatcher, and the duplicate `destructive-confirm` matcher registration
was merged. The dispatcher
keeps child scripts independent and fail-open; this reduces manifest and
runtime registration surface, while the child checks still run once per
SessionStart.
