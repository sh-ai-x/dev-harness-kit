...
| `contract.test` | `.github/workflows/contract-test-cron.yml` + `tools/contract_test_emitter.py` | Daily 06:00 UTC cron (also `workflow_dispatch`) runs the hermetic contract suite (`tests/test_harness_effectiveness.py` + `tests/test_harness_stability.py`) and emits one `contract.test` event with `outcome=passed|failed`. Stabilizes a per-day denominator so `stability.contract_test_pass_rate` converges toward statistical meaningfulness after ~7 days. Independent of `tests/conftest.py` — the cron is the canonical producer in CI; the conftest remains a parallel path for ad-hoc CI jobs that already run pytest. |
