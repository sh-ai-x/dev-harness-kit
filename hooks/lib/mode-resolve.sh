...
# _is_valid_mode <value> — returns 0 iff <value> ∈ {full,lite,undev,mod}.
...
# `mod` is the user-defined-role + dependency-aware-plan mode; see
# docs/scopes/modes.md for the semantics. The allow-list intentionally
# ships without any pre-defined `mod` subset — roles are operator-
# declared at /dev-kit:mode mod time, and the mode value alone is the
# gate (no extra hook surface wired here yet).
_is_valid_mode() {
  case "${1:-}" in
    full|lite|undev|mod) return 0 ;;
    *) return 1 ;;
  esac
}
