...
| `destructive-confirm` | all stages (not gated) | PreToolUse `Bash\|Write\|Edit\|MultiEdit` **ask-tier** gate — the only hook that emits `permissionDecision: "ask"` (human confirmation prompt) rather than deny-or-silence. Asks on: writes to `.env` / `*.pem` / `*.key` / `.ssh/*` / `.aws/credentials` / `.netrc` / `.kube/config` / `secrets.*` (`.env.example` and friends are exempt so the prompt stays meaningful); bare `git worktree remove` (bypasses `bin/worktree-remove-safe.sh`, discarding the worktree's `logs/`); `git push --force-with-lease`; and first-time `git push -u`. Opt out with `DEV_KIT_NO_CONFIRM=1`. First-time non-force `git push` ask can be bypassed loop-locally via `push_confirm=off` in `.dev-kit/guard-mode.session.json` (set by `/dev-kit:babysit-pr` and `/dev-kit:babysit-pr-local` on entry, restored to `on` in the EXIT trap); `--force-with-lease` is not bypassed. Fails closed when `jq` is missing. |
