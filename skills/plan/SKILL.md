...
### mod mode — dependency-aware step docs

When `DEV_KIT_MODE=mod` (read via `os.environ.get("DEV_KIT_MODE")` or
...
4. **Skip the `dependencies:` block in non-mod modes.** This section is
   gated on `DEV_KIT_MODE=mod`. In `full` / `lite` / `undev`, Gate 4/5
...
### mod mode — PRD §4 Dependency DAG subsection

When `DEV_KIT_MODE=mod`, extend PRD §4 (Phase plan) with a "Dependency
...
mod mode; in `full` / `lite` / `undev`, omit the subsection.

The DoD item list above (1-6) does not gain a 7th bullet in mod; the
