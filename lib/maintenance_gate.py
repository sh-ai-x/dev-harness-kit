...
# Manually maintained skill/command registry docs. When the PR adds a
# new ``skills/<name>/SKILL.md`` or ``commands/<name>.md``, it MUST also
# touch one of these (or carry the ``docs-not-required:`` marker in the
# PR body). Note: ``skills/README.md`` is auto-generated by
# ``tools/build_skills_readme.py`` and is intentionally EXCLUDED —
# counting it would make the gate a near-no-op (operators just run the
# regen tool). The whole point of this check is to force operators to
# also update the manually maintained sources below.
_REGISTRY_DOCS: frozenset = frozenset({
...
def registry_index_updated_ok(
    ...
    Pass conditions (any one):
      ...
      - The PR touches at least one of the manually maintained registry
        docs (``README.md``, ``docs/skills/README.md``,
        ``docs/skills/README.ko.md``, ``commands/README.md``).
      - The PR body carries the ``docs-not-required:`` marker.

    Fail: the PR adds a new skill or command, no registry doc is in
    the change set, and the PR body doesn't justify skipping. The
    reason names the first new skill/command AND lists the expected
    registry doc paths so the operator can fix the gate failure
    ...
    registry_touched = [
        ...
        if path in _REGISTRY_DOCS
    ]
    if registry_touched:
        return True, f"registry doc updated: {registry_touched[0]}"

    first = new_entries[0][0]
    expected = ", ".join(sorted(_REGISTRY_DOCS))
    return False, (
        f"PR added {first} but no registry doc was updated; "
        f"touch one of [{expected}] or quote an existing reference "
        f"in the PR body via docs-not-required:"
    )
...
def docs_updated_ok(
    ...
    (registry doc updated, or marker present), the docs sub-gate
    passes without falling through to the path-level ``docs/`` check
    — the manually maintained registry doc (e.g. top-level README.md)
    counts as the docs update.
    ...
    the specific new skill/command and the expected registry docs) is
    ...
    # diff, the operator MUST have touched a registry doc (or carry
    # the marker). The registry doc counts as the docs/ update, so
    # passing this check is sufficient — we don't ALSO require a
    # separate ``docs/`` file change.
    new_entries = [
        (p, s) for p, s in parsed
        if _is_new_skill(p, s) or _is_new_command(p, s)
    ]
    if new_entries:
        registry_touched = [p for p, _ in parsed if p in _REGISTRY_DOCS]
        if registry_touched:
            return True, f"registry doc updated: {registry_touched[0]}"
        first = new_entries[0][0]
        expected = ", ".join(sorted(_REGISTRY_DOCS))
